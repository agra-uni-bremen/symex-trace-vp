...
void _Pragma ( "entrypoint" ) quicksort_main( void )
{
  ...
  make_symbolic((void*) &quicksort_strings[1], 1);
  quicksort_str( *quicksort_strings, 3, sizeof( char[  20  ] ) );

  _Pragma( "marker recursivecall2" )
  _Pragma( "flowrestriction 1*quicksort_vec <= 650*recursivecall2" )
  make_symbolic((void*) &quicksort_vectors[1], 1);
  quicksort_vec(
    ( char * ) quicksort_vectors, 3,
...
int main( void )
{
  quicksort_init();
  quicksort_main();
  quicksort_return(); // We discard the return value, because we have to call symex_exit().
  symex_exit();
}
