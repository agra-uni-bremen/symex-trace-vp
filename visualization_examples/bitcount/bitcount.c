...
void bitcount_init()
{
  bitcount_randseed = 1;
  bitcount_n = 0;
  bitcount_iterations = 1; // Only one iteration to not overwork the symbolic execution engine.
...
unsigned long bitcount_random( void )
{
  ...
  make_symbolic(&symbol, sizeof(symbol));
  return symbol % 0x8;
  
  // The rest of the code is the original random generator.
...
int main( void )
{
  bitcount_init();
  bitcount_main();
  bitcount_return(); // Discard result because we require symex_exit() to be called.
  symex_exit();
}
