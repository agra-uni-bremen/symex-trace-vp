...
void _Pragma( "entrypoint" ) sha_main( void )
{
  ...
  fin.data = sha_data;
  make_symbolic((void*) &fin.data[0], 100 * sizeof(fin.data[0]));
  fin.size = 100;  // set size = 3247552 for input_large
  fin.cur_pos = 0;
...
int main ( void )
{
  sha_init();
  sha_main();
  sha_return(); // We discard the return value, because we have to call symex_exit().
  symex_exit();
}
