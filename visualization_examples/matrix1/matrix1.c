...
#define X 3 /* first dimension of array A */
#define Y 3 /* second dimension of array A, first dimension of array B */
#define Z 3 /* second dimension of array B */
...
void matrix1_pin_down( int A[  ], int B[  ], int C[  ] )
{
  ...
  for ( i = 0 ; i < X * Y; i++ )
    if (i == sizeof(matrix1_A) / sizeof(matrix1_A[0]) / 2 ) {
      make_symbolic((void*) &matrix1_A[i], sizeof(matrix1_A[i]) );
    } else {
      A[ i ] = x ;
    }
...
int main( void )
{
  matrix1_init();
  matrix1_main();
  matrix1_return(); // We discard the return value, because we have to call symex_exit().
  symex_exit();
}
