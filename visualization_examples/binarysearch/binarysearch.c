...
void binarysearch_init( void )
{
  ...
  for ( i = 0; i < 15; ++i ) {
    binarysearch_data[ i ].key = i; // Changed the key so that we are actually sorted.
...
{
  // We are guessing the search key.
  int search_key = 0;
  make_symbolic(&search_key, sizeof(search_key));
  for (int i = 0; i < 15; i++) {
    // We perform a search for every possible key, this way we hopefylly see something.
    if (search_key == i) {
      binarysearch_result = binarysearch_binary_search(i);
      symex_exit();
    }
  }
  symex_exit();
...
int main( void )
{
  binarysearch_init();
  binarysearch_main();
  binarysearch_return() - ( -1 ) != 0; // The return is discarded because we require symex_exit().
  symex_exit();
}
