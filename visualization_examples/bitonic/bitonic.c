...
#include "symex.h"
#define ARRAY_SIZE 16 // We parameterized the array size, to not overwork the solver.
...
int bitonic_a[ ARRAY_SIZE ];         // the array to be sorted
...
void bitonic_init( void )
{
  ...
  _Pragma( "loopbound min ARRAY_SIZE max ARRAY_SIZE" )
  for ( i = 0; i < ARRAY_SIZE; i++ )
    if (i == ARRAY_SIZE / 2) {
      // We want to make a single value symbolic.
      make_symbolic(&bitonic_a[i], sizeof(bitonic_a[i]));
    } else {
      bitonic_a[ i ] = ( ARRAY_SIZE - i );
    }
}
...
int bitonic_return( void )
{
  ...
  checksum += bitonic_a[ 0 ] + bitonic_a[ ARRAY_SIZE / 2 ] + bitonic_a[ ARRAY_SIZE - 1 ];
...
void bitonic_merge( int lo, int cnt, int dir )
{
  int k = cnt / 2;
  int i;
  _Pragma( "loopbound min 0 max (ARRAY_SIZE / 2)" )
  for ( i = lo; i < lo + k; i++ )
...
void bitonic_sort( int lo, int cnt, int dir )
{
  ...
  _Pragma( "flowrestriction 1*bitonicMerge <= (ARRAY_SIZE - 1)*recMerge" )
...
void _Pragma ( "entrypoint" ) bitonic_main( void )
{
  ...
  bitonic_sort( 0, ARRAY_SIZE, bitonic_ASCENDING );
...
int main( void )
{
  bitonic_init();
  bitonic_main();
  bitonic_return(); // Discard result because we require symex_exit() to be called.
  symex_exit();
}
