...
// We reduced the size of bsort to not overwork the solver. Original value was 100.
#define bsort_SIZE 8
...
int bsort_Initialize( int Array[] )
{
  ...
  _Pragma( "loopbound min bsort_SIZE max bsort_SIZE" )
  for ( Index = 0; Index < bsort_SIZE; Index ++ )
    if (Index == bsort_SIZE / 2) {
      // We want to make a single value symbolic.
      make_symbolic(&Array[Index], sizeof(Array[Index]));
    } else {
...
int bsort_return( void )
{
  ...
  _Pragma( "loopbound min (bsort_SIZE - 1) max (bsort_SIZE - 1)" )
...
int bsort_BubbleSort( int Array[] )
{
  ...
  for ( i = 0; i < bsort_SIZE - 1; i ++ ) {
    Sorted = 1;
    _Pragma( "loopbound min 3 max (bsort_SIZE - 1)" )
    for ( Index = 0; Index < bsort_SIZE - 1; Index ++ ) {
...
int main( void )
{
  bsort_init();
  bsort_main();
  bsort_return(); // Discard result because we require symex_exit() to be called.
  symex_exit();
}
